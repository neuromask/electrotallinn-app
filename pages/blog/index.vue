    ...
    <transition-group name="card-list" mode="out-in" class="row">
      <b-col cols="12" md="6" lg="4" class="card-list-item mb-4" v-for="post in posts" :key="post.title">
        <b-card class="shadow-sm" bg-variant="info" style="font-size:0.9rem; line-height:1rem;" >
          <div class="p-container mb-3 position-relative">
          ...
          <b-button size="sm" block variant="primary" class="text-info" nuxt :to="localePath(post.path)"><b-icon icon="search" /> {{ $t('blog.readMore') }}</b-button>
        </b-card>
      </b-col>
    </transition-group>
  </section>
