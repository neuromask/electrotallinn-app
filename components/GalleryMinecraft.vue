...
export default {
  ...
  mounted() {
    this.importAll(require.context("@/assets/img/minecraft/gallery", false, /\.jpg$/));
  },
  methods: {
    importAll(r) {
      r.keys().forEach(key => (this.images.push({ src: r(key), thumbnail: r(key), caption: "Minecraft ElectroTallinn Server" })));
    }
  },
};
</script>