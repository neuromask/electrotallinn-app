...
module.exports = {
    findAll: async () => {
        let sql = 'SELECT id, first_name, uin, photo_url, username, role, birthyear, languages, location, transport_model, transport_photo_name AS transport_photo_name, (SELECT count(l.id) FROM locations l WHERE l.user_uin = uin) AS locations_count, (SELECT count(mp.id) FROM market_products mp WHERE mp.user_uin = uin AND mp.status = \'ACTIVE\') AS market_products_count FROM users';
    ...
    findByUin: async (uin) => {
        let sql = 'SELECT first_name, username, uin, photo_url, role, birthyear, languages, location, transport_model, transport_photo_name, (SELECT count(l.id) FROM locations l WHERE l.user_uin = uin) AS locations_count, (SELECT count(mp.id) FROM market_products mp WHERE mp.user_uin = uin AND mp.status = \'ACTIVE\') AS market_products_count FROM users WHERE uin = ?';
        let params = [uin];
